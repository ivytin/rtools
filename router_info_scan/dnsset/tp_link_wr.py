...
class DnsSetter(BaseSetter):
    ...
    def dns_set(self, dns):
        ...
        if wan_url.find('WanDynamic') > 0:
            ...
            try:
                r = self.connect_auth_with_headers(wan_url, 2)
            ...
            payload = self.static_payload(dns[0], dns[1], static_ip, static_mask, static_gateway)
            dns_url = wan_url + payload
            if self.debug:
    ...
    def static_payload(self, dns1, dns2, ip, mask, gatewaty):
