...
class DnsSetter(BaseSetter):
    ...
    def get_now_info(self, type):
        ...
        elif type == pppoe_bas_url:
            jump_url = ''
        else:
            jump_url = dyna_bas_url
        r = self.connect_auth_with_headers(jump_url, 3)
        return r.content
    ...
    def pppoe_payload(self, content, dns):
        pppoe_payload = dict()
        ...
        if pppoe_match['mac_assign'] == '0':
            ...
            pppoe_payload['mac_assign'] += '2' + pppoe_payload['spoof_mac'] + pppoe_match['spoof_mac'].replace('.', '%3A')
        ...
        return pppoe_payload['head'] + pppoe_payload['pppoe_ipaddr'] + pppoe_payload['pppoe_subnet'] + \
               pppoe_payload['pppoe_dnsaddr1'] + pppoe_payload['pppoe_dnsaddr2'] + pppoe_payload['hidden_pppoe_idle_time'] + \
    ...
    def dyna_payload(self, content, dns):
        dyna_payload = dict()
    ...
    def generate_payload(self, content, dns, type):
        if type == 'pppoe':
            return self.pppoe_payload(content, dns)
        elif type == 'dyna':
            return self.dyna_payload(content, dns)
    ...
    def dns_set(self, dns):
        ...
        payload = self.generate_payload(content, dns, type)
        if self.debug:
